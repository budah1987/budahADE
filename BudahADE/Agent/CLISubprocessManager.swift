...
final class CLISubprocessManager: ObservableObject {
    ...
    func send(sessionId: UUID, prompt: String, model: AgentModel? = nil, showInChat: Bool = true, attachments: [DocumentAttachment] = []) {
        ...
        if showInChat {
            session.addUserMessage(prompt, attachments: attachments) // Show original prompt in UI
        }
